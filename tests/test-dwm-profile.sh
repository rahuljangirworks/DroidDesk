...
grep -q 'private const val PROFILE_VERSION = "2"' "$profile"
grep -q '9e895f3723825120687e1ecf2404d53ed83d677d' "$profile"
...
grep -q 'env\["GIT_EXEC_PATH"\].*libexec/git-core' "$linux_runtime"
grep -q 'env\["GIT_TEMPLATE_DIR"\].*share/git-core/templates' "$linux_runtime"
grep -q 'env\["PKG_CONFIG_LIBDIR"\]' "$linux_runtime"
grep -q 'listOf("bin", "lib", "libexec", "share", "etc", "var/lib/dpkg/info")' "$linux_runtime"
grep -q "SHELL=\"\${binDir.absolutePath}/sh\"" "$linux_runtime"
grep -q 'checkout --detach --force' "$linux_runtime"
grep -q 'checkout --detach --force' "$chroot_runtime"
grep -q '"xkeyboard-config"' "$profile"
grep -q '"libxcursor"' "$profile"
grep -q '"quickshell"' "$profile"
grep -q 'destination = autostartDir' "$profile"
grep -q 'destination = File(profileData, "config")' "$profile"
